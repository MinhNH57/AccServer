...
namespace Catalog.SGas.Features;
...
public class Api : ICarterModule
{
    public void AddRoutes(IEndpointRouteBuilder app)
    {
        ...
        var api = vApi.MapGroup("catalog/sgas/").HasApiVersion(1.0).RequireAuthorization();
    ...
    private async Task<IResult> DynamicAddListData(
        [AsParameters] CatalogSGasService service,
    ...
    private Type GetEntityType(string entityType)
    {
        ...
        var assemblyData = Assembly.Load("Catalog.SGas"); // Tên assembly chứa các model
        ...
        //var typeHrm = assemblyFund.GetTypes().FirstOrDefault(t => t.Name == entityType);
        //if (typeHrm is not null) return typeHrm;


        throw new InvalidOperationException($"Type '{entityType}' không tồn tại trong assembly 'Catalog'.");
    ...
    private async Task<IResult> GetViewAsync([AsParameters] CatalogSGasService service,string typeSize)
    {
