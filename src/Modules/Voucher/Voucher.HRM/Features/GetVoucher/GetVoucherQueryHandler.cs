...
using Voucher.HRM.Features.Queries.GetVoucher;
using Voucher.HRM.Infrastructure;

namespace Voucher.HRM.Features.Queries.GetVoucher;

public class GetVoucherQueryHandler(VoucherHrmDbContext dbContext) : IQueryHandler<GetVoucherQuery, ApiResponse<List<SmartDataApplication>>>
{
    public async Task<ApiResponse<List<SmartDataApplication>>> Handle(GetVoucherQuery query, CancellationToken cancellationToken)
    {
        var root = (IQueryable<SmartDataApplication>)dbContext.SmartDataApplication.AsNoTracking().OrderByDescending(c=>c.RecordDate);
        ...
        if (!string.IsNullOrEmpty(query.Filtering.Fields))
        {
            ...
            root = (IQueryable<SmartDataApplication>)root.Select(fields);
        }

        var smartDatas = await root.ToListAsync(cancellationToken: cancellationToken);

        return ApiResponseFactory<List<SmartDataApplication>>.Ok(smartDatas);
