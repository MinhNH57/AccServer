...
public class GetProductionPlanQueryHandler(VoucherDbContext dbContext) : IQueryHandler<GetProductionPlanQuery, ApiResponse<List<SmartProductionPlan>>>
{
    public async Task<ApiResponse<List<SmartProductionPlan>>> Handle(GetProductionPlanQuery query, CancellationToken cancellationToken)
    {
        var root = (IQueryable<SmartProductionPlan>)dbContext.SmartProductionPlans.AsNoTracking().OrderByDescending(c=>c.NumberOfVouchers);
        ...
        if (!string.IsNullOrEmpty(query.Filtering.Fields))
        {
            ...
            root = (IQueryable<SmartProductionPlan>)root.Select(fields);
        }

        var smartDatas = await root.ToListAsync(cancellationToken: cancellationToken);

        return ApiResponseFactory<List<SmartProductionPlan>>.Ok(smartDatas);
