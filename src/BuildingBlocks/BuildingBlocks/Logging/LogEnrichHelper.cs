...
public static class LogEnrichHelper
{
    ...
    public static void EnrichFromRequest(IDiagnosticContext diagnosticContext, HttpContext httpContext)
    {
        ...
        request.Headers.TryGetValue("X-Correlation-ID", out var data);
        diagnosticContext.Set("CorrelationId", data.ToString());
        ...
        var tenantIdValue = request.HttpContext.User.FindFirstValue(ClaimTypeCustom.TenantId);
        if (string.IsNullOrEmpty(tenantIdValue))
        {
            tenantIdValue = request.HttpContext.Request.Headers.TryGetValue("X-Tenant-Id", out var tenantId) ? tenantId.ToString() : "";
        }

        diagnosticContext.Set("TenantId", tenantIdValue);
    }
}
